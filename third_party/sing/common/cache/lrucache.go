...
// DeleteIf removes the current value only when match accepts it. match runs
// under the cache lock and must not call the cache. Eviction runs after removal
// and unlocking, so closing an old value cannot remove its replacement.
