...
func lifecycleConn() *conn {
	...
	c := &conn{ctx: ctx, cancel: cancel, data: make(chan packet, 64), ready: make(chan struct{}), readDeadline: pipe.MakeDeadline()}
	close(c.ready)
	return c
}
...
func TestCloseUnblocksFullQueueAndReleasesPackets(t *testing.T) {
	...
	for i := 0; i < 64; i++ {
		...
		if !c.enqueue(packet{data: packets[i]}) {
			t.Fatal("enqueue failed")
		}
	}
	packets[64] = lifecycleBuffer()
	done := make(chan bool, 1)
	go func() { done <- c.enqueue(packet{data: packets[64]}) }()
	...
	case accepted := <-done:
		if accepted {
			t.Fatal("post-close packet accepted")
		}
	case <-time.After(3 * time.Second):
